...
2. texto         padronizar (+ mapa de sinônimos)  -> confira: sobrou o número certo de categorias?
3. números       para_numero            -> confira: nada virou vazio por causa de vírgula ou R$?
...
`format="mixed", dayfirst=True` (Aula 10) **inverte dia e mês** das datas `2026-08-05` e `2026/08/05` no pandas 3, sem dar erro. Use o `converter_data` (seção 4 do template) e confira `min()` e `max()` depois.

## Número com vírgula — a outra armadilha

`pd.to_numeric("347,9", errors="coerce")` dá **vazio**, sem erro. Se a coluna inteira veio com vírgula (ou `R$`), ela some toda. Use o `para_numero` (seção 4 do template).
