...
```python
print(df.dtypes)                              # o que veio como texto e deveria ser número/data?
print(df["tema"].unique())                    # quais grafias diferentes existem?
print(df["tema"].value_counts(dropna=False))  # quantas de cada, incluindo ausentes
print(df.duplicated(subset="id_publicacao").sum())   # tem duplicata?
print(df.describe())                          # min/max ajudam a achar valor inválido (negativo, zero)
```
