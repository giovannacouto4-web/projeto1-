...
def main():
    ...
    checagens = [
        ("limpeza: tema com 4 categorias (acento e caixa unificados)", lambda: ns["df"]["tema"].nunique() == 4),
        ("limpeza: salvamentos com vírgula viraram número", lambda: ns["df"]["salvamentos"].notna().all()
         and ns["df"]["salvamentos"].dtype.kind == "f"),
        ("para_numero: R$, vírgula e texto", lambda: ns["para_numero"](__import__("pandas").Series(
            ["R$ 1.234,56", "347,9", "sem dado", "12"])).round(2).fillna(-1).tolist() == [1234.56, 347.9, -1, 12]),
        ("converter_data: 3 formatos, mesmo dia", lambda: ns["converter_data"](__import__("pandas").Series(
            ["2026/02/06 09:15", "06/02/2026 09:15", "2026-02-06 09:15"])).nunique() == 1
         and ns["converter_data"](__import__("pandas").Series(["2026/02/06"])).dt.month.iloc[0] == 2),
        ("padronizar: Saúde/SAUDE/saude viram uma", lambda: ns["padronizar"](__import__("pandas").Series(
            [" Saúde ", "SAUDE", "saude"])).nunique() == 1),
        ...
        ("classificação: modelo bobo com recall 0", lambda: ns["tabela_bobo"]["recall"].iloc[0] == 0),
