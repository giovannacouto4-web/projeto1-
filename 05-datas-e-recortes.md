...
```python
print(df["data_publicacao"].dt.date)          # só a data (vira objeto date)
print(df["data_publicacao"].dt.day)           # dia do mês
print(df["data_publicacao"].dt.month)         # mês
print(df["data_publicacao"].dt.hour)          # hora
print(df["data_publicacao"].dt.dayofweek)     # 0 = segunda ... 6 = domingo
print(df["data_publicacao"].dt.day_name())    # "Monday", "Tuesday"...
print(df["data_publicacao"].dt.isocalendar().week)   # número da semana
print(df["data_publicacao"].dt.to_period("W"))       # agrupar por semana
```
...
```python
print(df[df["tema"].isin(["Cultura", "Saúde"])])         # em uma lista
print(df[df["tema"].str.contains("Cult", na=False)])      # contém texto
print(df[df["data_publicacao"] >= "2026-09-01"])          # a partir de uma data
print(df[df["taxa_engajamento_pct"].between(5, 10)])     # intervalo (inclui as pontas)
```
