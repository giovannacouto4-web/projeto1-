...
df["tema"] = padronizar(df["tema"])                  # caixa, espaço E acento: "Saúde"/"saude" viram uma
print(sorted(df["tema"].dropna().unique()))          # sobrou sinônimo? mapeie: df["tema"].replace({"poda": "poda preventiva"})
for col in ["alcance", "compartilhamentos", "salvamentos"]:
    df[col] = para_numero(df[col])                   # aceita "347,9" e "R$ 1.234,56" (to_numeric sozinho zeraria)
...
print(rec.nlargest(5, "taxa_engajamento_pct")[["id_publicacao", "dia_publicacao", "hora", "tema", "taxa_engajamento_pct"]])
```
