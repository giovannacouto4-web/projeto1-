...
|---|---|---|
| Aparecem 7+ temas quando deveriam ser 4 | faltou padronizar texto (caixa, espaço, **acento**: `saude` × `Saúde`) | `padronizar` (seção 4 do template) |
| Depois de padronizar, ainda sobram categorias a mais | sinônimo ou abreviação (`poda` × `poda preventiva`, `app` × `aplicativo`) | mapa manual com `.replace({...})`, escrito a partir do `unique()` |
| Uma coluna numérica virou quase toda vazia | número com vírgula decimal (`"347,9"`) ou `R$`: `to_numeric` transforma em `NaN` sem avisar | `para_numero` (seção 4 do template) |
| Datas fora do período da campanha | pandas 3: `format="mixed", dayfirst=True` (Aula 10) inverteu dia e mês das datas que começam pelo ano (`2026-08-05`, `2026/08/05`) com dia ≤ 12 | `converter_data` (`02-limpeza.md`, seção 4 do template) |
...
```python
print(pd.Series(modelo.coef_[0], index=X_treino.columns).abs().sort_values(ascending=False))
```
Se estiver dentro de um pipeline:
```python
print(modelo.named_steps["logisticregression"].coef_[0])
```
...
```python
print(df[(df["formato"] == "reel") & (df["hora"] >= 18)])   # correto
```
...
print(df.sample(5))                                    # linhas aleatórias
```
