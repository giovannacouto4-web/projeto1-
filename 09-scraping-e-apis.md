...
**No site da prova (Python no navegador):** `requests` e Playwright não funcionam lá — não há acesso à internet nem navegador controlável. Se cair HTML, ele virá como texto ou arquivo: use só o BeautifulSoup (seção 12 do template, que já parte de um HTML em texto).
...
print(sopa.find(id="titulo-site"))                                   # por id
link = primeiro.find("a", {"class": "link-post"})
print(link.get_text(strip=True))                                     # texto visível, sem espaços nas pontas
print(link["href"])                                                  # atributo
```
